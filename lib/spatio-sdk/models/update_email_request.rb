...
module Spatio
  ...
  class UpdateEmailRequest < ApiModelBase
    ...
    attr_accessor :add_labels

    attr_accessor :remove_labels
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::UpdateEmailRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::UpdateEmailRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'add_labels')
        if (value = attributes[:'add_labels']).is_a?(Array)
          self.add_labels = value
        end
      end

      if attributes.key?(:'remove_labels')
        if (value = attributes[:'remove_labels']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          add_labels == o.add_labels &&
          remove_labels == o.remove_labels
    end
    ...
    def hash
      [account_id, is_read, is_starred, add_labels, remove_labels].hash
    end
