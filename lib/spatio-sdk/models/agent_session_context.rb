...
module Spatio
  ...
  class AgentSessionContext < ApiModelBase
    attr_accessor :user
    ...
    def self.openapi_types
      {
        ...
        :'current_organization' => :'Hash<String, Object>',
        :'current_workspace' => :'Hash<String, Object>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::AgentSessionContext` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::AgentSessionContext`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'current_organization')
        if (value = attributes[:'current_organization']).is_a?(Hash)
          self.current_organization = value
        end
      end

      if attributes.key?(:'current_workspace')
        if (value = attributes[:'current_workspace']).is_a?(Hash)
          self.current_workspace = value
        end
      end
    ...
    def hash
      [user, current_organization, current_workspace, connected_accounts].hash
    end
