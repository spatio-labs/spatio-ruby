...
module Spatio
  ...
  class PreconfiguredAgent < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::PreconfiguredAgent` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::PreconfiguredAgent`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'has_all_tools')
        self.has_all_tools = attributes[:'has_all_tools']
      end

      if attributes.key?(:'tool_count')
        self.tool_count = attributes[:'tool_count']
      end
    end
    ...
    def hash
      [agent_id, name, tagline, description, icon, has_all_tools, tool_count].hash
    end
