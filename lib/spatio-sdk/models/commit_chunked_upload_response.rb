=begin
#SpatioAPI

#The REST API that owns every resource in your Spatio workspace: notes, sheets, slides, tasks, calendar events, mail, chat, files, and contacts. SpatioMCP wraps this API; Spatio Desktop reads from it. You can call it directly from your own code.  All requests must be authenticated with a Personal Access Token (`Authorization: Bearer pat_...`) or an OAuth 2.1 access token, and use HTTPS.  Official SDKs (MIT, generated from this spec on every release):  - TypeScript: https://github.com/spatio-labs/spatio-ts (`npm install @spatio-labs/spatio-ts`) - Python: https://github.com/spatio-labs/spatio-py (`pip install spatio-sdk`) - Go: https://github.com/spatio-labs/spatio-go (`go get github.com/spatio-labs/spatio-go`)  This specification is generated from the platform-service Go source on every push to `main`. The spec, not hand-written documentation, is the source of truth: server stubs and SDKs are generated from it, and any drift between the spec and the running service fails CI. 

The version of the OpenAPI document: v1
Contact: hello@spatio.app
Generated by: https://openapi-generator.tech
Generator version: 7.22.0

=end

require 'date'
require 'time'

module Spatio
  class CommitChunkedUploadResponse < ApiModelBase
    attr_accessor :success

    attr_accessor :file_id

    attr_accessor :manifest_id

    attr_accessor :version

    attr_accessor :total_size

    attr_accessor :physical_size

    attr_accessor :deduplication_pct

    attr_accessor :total_blocks

    attr_accessor :new_blocks

    attr_accessor :deduplicated_blocks

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'success' => :'success',
        :'file_id' => :'fileId',
        :'manifest_id' => :'manifestId',
        :'version' => :'version',
        :'total_size' => :'totalSize',
        :'physical_size' => :'physicalSize',
        :'deduplication_pct' => :'deduplicationPct',
        :'total_blocks' => :'totalBlocks',
        :'new_blocks' => :'newBlocks',
        :'deduplicated_blocks' => :'deduplicatedBlocks'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'success' => :'Boolean',
        :'file_id' => :'String',
        :'manifest_id' => :'String',
        :'version' => :'Integer',
        :'total_size' => :'Integer',
        :'physical_size' => :'Integer',
        :'deduplication_pct' => :'Float',
        :'total_blocks' => :'Integer',
        :'new_blocks' => :'Integer',
        :'deduplicated_blocks' => :'Integer'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::CommitChunkedUploadResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::CommitChunkedUploadResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'success')
        self.success = attributes[:'success']
      else
        self.success = nil
      end

      if attributes.key?(:'file_id')
        self.file_id = attributes[:'file_id']
      else
        self.file_id = nil
      end

      if attributes.key?(:'manifest_id')
        self.manifest_id = attributes[:'manifest_id']
      else
        self.manifest_id = nil
      end

      if attributes.key?(:'version')
        self.version = attributes[:'version']
      else
        self.version = nil
      end

      if attributes.key?(:'total_size')
        self.total_size = attributes[:'total_size']
      else
        self.total_size = nil
      end

      if attributes.key?(:'physical_size')
        self.physical_size = attributes[:'physical_size']
      end

      if attributes.key?(:'deduplication_pct')
        self.deduplication_pct = attributes[:'deduplication_pct']
      end

      if attributes.key?(:'total_blocks')
        self.total_blocks = attributes[:'total_blocks']
      end

      if attributes.key?(:'new_blocks')
        self.new_blocks = attributes[:'new_blocks']
      end

      if attributes.key?(:'deduplicated_blocks')
        self.deduplicated_blocks = attributes[:'deduplicated_blocks']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      if @success.nil?
        invalid_properties.push('invalid value for "success", success cannot be nil.')
      end

      if @file_id.nil?
        invalid_properties.push('invalid value for "file_id", file_id cannot be nil.')
      end

      if @manifest_id.nil?
        invalid_properties.push('invalid value for "manifest_id", manifest_id cannot be nil.')
      end

      if @version.nil?
        invalid_properties.push('invalid value for "version", version cannot be nil.')
      end

      if @total_size.nil?
        invalid_properties.push('invalid value for "total_size", total_size cannot be nil.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      return false if @success.nil?
      return false if @file_id.nil?
      return false if @manifest_id.nil?
      return false if @version.nil?
      return false if @total_size.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] success Value to be assigned
    def success=(success)
      if success.nil?
        fail ArgumentError, 'success cannot be nil'
      end

      @success = success
    end

    # Custom attribute writer method with validation
    # @param [Object] file_id Value to be assigned
    def file_id=(file_id)
      if file_id.nil?
        fail ArgumentError, 'file_id cannot be nil'
      end

      @file_id = file_id
    end

    # Custom attribute writer method with validation
    # @param [Object] manifest_id Value to be assigned
    def manifest_id=(manifest_id)
      if manifest_id.nil?
        fail ArgumentError, 'manifest_id cannot be nil'
      end

      @manifest_id = manifest_id
    end

    # Custom attribute writer method with validation
    # @param [Object] version Value to be assigned
    def version=(version)
      if version.nil?
        fail ArgumentError, 'version cannot be nil'
      end

      @version = version
    end

    # Custom attribute writer method with validation
    # @param [Object] total_size Value to be assigned
    def total_size=(total_size)
      if total_size.nil?
        fail ArgumentError, 'total_size cannot be nil'
      end

      @total_size = total_size
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          success == o.success &&
          file_id == o.file_id &&
          manifest_id == o.manifest_id &&
          version == o.version &&
          total_size == o.total_size &&
          physical_size == o.physical_size &&
          deduplication_pct == o.deduplication_pct &&
          total_blocks == o.total_blocks &&
          new_blocks == o.new_blocks &&
          deduplicated_blocks == o.deduplicated_blocks
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [success, file_id, manifest_id, version, total_size, physical_size, deduplication_pct, total_blocks, new_blocks, deduplicated_blocks].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

  end

end
