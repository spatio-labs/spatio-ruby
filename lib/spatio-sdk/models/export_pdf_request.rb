...
module Spatio
  ...
  class ExportPDFRequest < ApiModelBase
    attr_accessor :rasterized_slides
    ...
    def self.openapi_types
      {
        :'rasterized_slides' => :'Array<ExportPDFRequestRasterizedSlidesInner>',
        :'theme' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::ExportPDFRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::ExportPDFRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'rasterized_slides')
        if (value = attributes[:'rasterized_slides']).is_a?(Array)
          self.rasterized_slides = value
        end
      end

      if attributes.key?(:'theme')
        if (value = attributes[:'theme']).is_a?(Hash)
          self.theme = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          rasterized_slides == o.rasterized_slides &&
          theme == o.theme
    end
