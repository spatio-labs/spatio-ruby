...
module Spatio
  ...
  class DMForwardRequest < ApiModelBase
    attr_accessor :to_dm_id

    attr_accessor :to_channel_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::DMForwardRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::DMForwardRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'to_dm_id')
        self.to_dm_id = attributes[:'to_dm_id']
      end

      if attributes.key?(:'to_channel_id')
        self.to_channel_id = attributes[:'to_channel_id']
      end
    ...
    def hash
      [to_dm_id, to_channel_id, account_id].hash
    end
