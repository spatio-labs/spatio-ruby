...
module Spatio
  class WorkspaceMember < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::WorkspaceMember` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::WorkspaceMember`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      role_validator = EnumAttributeValidator.new('String', ["owner", "admin", "member", "guest"])
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["owner", "admin", "member", "guest"])
    ...
    def hash
      [id, role, email, name, avatar, joined_at, user].hash
    end
