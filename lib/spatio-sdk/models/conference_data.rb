...
module Spatio
  ...
  class ConferenceData < ApiModelBase
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::ConferenceData` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::ConferenceData`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dial_in')
        if (value = attributes[:'dial_in']).is_a?(Array)
          self.dial_in = value
        end
      end
    end
    ...
    def hash
      [type, uri, meeting_id, passcode, access_code, dial_in].hash
    end
