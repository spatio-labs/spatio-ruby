  ...
  class UpdateEventRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::UpdateEventRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::UpdateEventRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] send_updates Object to be assigned
    def send_updates=(send_updates)
      validator = EnumAttributeValidator.new('String', ["all", "externalOnly", "none"])
      unless validator.valid?(send_updates)
        fail ArgumentError, "invalid value for \"send_updates\", must be one of #{validator.allowable_values}."
      end
      @send_updates = send_updates
    end
    ...
    def hash
      [account_id, updates, send_updates].hash
    end
