...
module Spatio
  ...
  class CalendarSyncResponse < ApiModelBase
    attr_accessor :enqueued

    attr_accessor :jobs

    attr_accessor :waited
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::CalendarSyncResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::CalendarSyncResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'enqueued')
      ...
      if attributes.key?(:'waited')
        self.waited = attributes[:'waited']
      end
    ...
    def list_invalid_properties
      ...
      if @enqueued.nil?
        invalid_properties.push('invalid value for "enqueued", enqueued cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @enqueued.nil?
      return false if @jobs.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] enqueued Value to be assigned
    def enqueued=(enqueued)
      if enqueued.nil?
        fail ArgumentError, 'enqueued cannot be nil'
      end
    ...
    def hash
      [enqueued, jobs, waited, timed_out, errors].hash
    end
