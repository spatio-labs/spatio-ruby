...
module Spatio
  class ExportPDFRequestRasterizedSlidesInner < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::ExportPDFRequestRasterizedSlidesInner` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::ExportPDFRequestRasterizedSlidesInner`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @png.nil?
        invalid_properties.push('invalid value for "png", png cannot be nil.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] png Value to be assigned
    def png=(png)
      if png.nil?
        fail ArgumentError, 'png cannot be nil'
      end
