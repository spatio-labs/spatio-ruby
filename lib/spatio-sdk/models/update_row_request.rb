...
module Spatio
  class UpdateRowRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::UpdateRowRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::UpdateRowRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'cells')
        if (value = attributes[:'cells']).is_a?(Hash)
          self.cells = value
        end
      else
        self.cells = nil
    ...
    # @param [Object] cells Value to be assigned
    def cells=(cells)
      if cells.nil?
        fail ArgumentError, 'cells cannot be nil'
      end
