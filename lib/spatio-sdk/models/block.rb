  ...
  class Block < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::Block` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::Block`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] has_children Value to be assigned
    def has_children=(has_children)
      if has_children.nil?
        fail ArgumentError, 'has_children cannot be nil'
      end
    ...
    def hash
      [id, note_id, parent_id, type, content, properties, position, has_children, created_at, updated_at].hash
    end
