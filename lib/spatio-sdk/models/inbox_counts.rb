...
module Spatio
  class InboxCounts < ApiModelBase
    attr_accessor :total
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::InboxCounts` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::InboxCounts`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @mentions.nil?
        invalid_properties.push('invalid value for "mentions", mentions cannot be nil.')
      end
    ...
    def mentions=(mentions)
      if mentions.nil?
        fail ArgumentError, 'mentions cannot be nil'
      end
    ...
    def hash
      [total, mentions, notifications].hash
    end
