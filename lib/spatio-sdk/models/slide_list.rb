  ...
  class SlideList < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::SlideList` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::SlideList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'slides')
    ...
    def list_invalid_properties
      ...
      if @slides.nil?
        invalid_properties.push('invalid value for "slides", slides cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @slides.nil?
      return false if @total.nil?
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] slides Value to be assigned
    def slides=(slides)
      if slides.nil?
        fail ArgumentError, 'slides cannot be nil'
      end
