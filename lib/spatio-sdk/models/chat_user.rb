  ...
  class ChatUser < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Spatio::ChatUser` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Spatio::ChatUser`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] is_bot Value to be assigned
    def is_bot=(is_bot)
      if is_bot.nil?
        fail ArgumentError, 'is_bot cannot be nil'
      end
    ...
    def hash
      [id, provider, account_id, name, real_name, email, avatar, is_bot, is_active].hash
    end
